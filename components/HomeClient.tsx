...
function StatItem({ target, label }: { target: number; label: string }) {
  ...
  return (
    <div ref={ref} className="text-center px-8 sm:px-14 py-2">
      ...
      <div className="text-[11px] mt-2 tracking-[0.22em] uppercase" style={{ color: 'rgba(255,255,255,0.3)' }}>
        {label}
      </div>
    </div>
  )
  ...
  return (
    <div className="relative -mt-8">
      ...
      <section className="relative text-center pt-24 pb-20 sm:pt-32 sm:pb-28 overflow-hidden">
        <div className="absolute inset-x-0 bottom-0 h-px" style={{ background: 'rgba(255,255,255,0.08)' }} />

        <div className="animate-gavel animate-hero-in text-6xl sm:text-8xl mb-8 select-none">⚖️</div>

        <h1
          className="animate-hero-in text-5xl sm:text-7xl font-black tracking-tight mb-3 text-white"
        ...
        <div className="animate-fade-up-2 h-9 mb-10 flex items-center justify-center">
          <p
            ...
            style={{ fontFamily: "'Noto Serif KR', serif", color: 'rgba(255,255,255,0.65)' }}
          >
            {typingText}
        ...
        <div className="animate-fade-up-3 flex flex-col sm:flex-row items-center justify-center gap-3">
          <Link href="/cases/new" className="btn-glass">⚖️ 지금 사건 접수하기</Link>
          <Link href="/laws" className="btn-ghost">📜 법전 보기</Link>
        </div>

        <div className="animate-fade-up-4 mt-16 flex items-center justify-center gap-4">
          <div className="h-px w-14" style={{ background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.12))' }} />
          <span className="text-[9px] tracking-[0.45em] uppercase" style={{ color: 'rgba(255,255,255,0.18)', fontWeight: 300 }}>
            The Court Is In Session
          </span>
          <div className="h-px w-14" style={{ background: 'linear-gradient(270deg, transparent, rgba(255,255,255,0.12))' }} />
        </div>
      </section>

      {/* ── 통계 ── */}
      <FadeIn>
        <section className="relative py-10 mb-14" style={{ borderTop: '1px solid rgba(255,255,255,0.08)', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
      ...
      <FadeIn delay={60}>
        <section className="mb-16">
          <div className="flex items-center justify-center gap-4 mb-9">
            ...
            <p className="text-[10px] tracking-[0.4em] uppercase" style={{ color: 'rgba(255,255,255,0.25)', fontWeight: 300 }}>
              How It Works
            </p>
            <div className="h-px flex-1" style={{ background: 'rgba(255,255,255,0.08)' }} />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { icon: '📝', step: '01', title: '사건 접수',    desc: '억울한 사연을 상세히 제출하세요. 원고·피고 양측의 진술을 기록합니다.' },
              { icon: '🤖', step: '02', title: 'AI 판사 심판', desc: 'Claude AI가 관계법정 법전에 근거하여 공정하고 권위 있는 판결을 내립니다.' },
              { icon: '🗳️', step: '03', title: '배심원 투표',  desc: '전체 유저가 배심원이 되어 원고·피고·무승부 중 하나를 선택합니다.' },
            ].map(({ icon, step, title, desc }) => (
              <div key={step} className="step-card group">
                <div className="text-[10px] font-medium tracking-[0.35em] mb-4" style={{ color: 'rgba(255,255,255,0.2)' }}>
                  STEP {step}
                </div>
                <div className="text-4xl mb-4 transition-transform duration-300 group-hover:scale-110">{icon}</div>
                <div className="font-bold text-[15px] mb-3 text-white" style={{ fontFamily: "'Noto Serif KR', serif" }}>
                  {title}
                </div>
      ...
      <FadeIn delay={80}>
        <section>
          <div className="flex items-end justify-between mb-5">
            <div>
              <h2 className="text-[17px] font-bold text-white" style={{ fontFamily: "'Noto Serif KR', serif" }}>
                진행 중인 사건
              </h2>
          ...
          {cases.length > 0 ? (
            ...
            <div className="case-card-glow p-16 text-center">
              ...
              <p className="text-sm" style={{ color: 'rgba(255,255,255,0.3)' }}>아직 접수된 사건이 없습니다</p>
              <Link href="/cases/new" className="inline-block mt-4 text-sm text-white underline underline-offset-4">
                첫 사건을 접수해보세요 →
              </Link>
            </div>
          )}
